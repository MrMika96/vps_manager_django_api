...
def test_update_personal_data_request(api_client_with_credentials, test_user_profile):
    ...
    response = api_client_with_credentials.put(
        url, data={"profile": test_user_profile}, format="json"
    )
...
def test_user_registration_request(
    api_client, test_user_profile, test_password, test_email
):
...
def test_user_auth_request(api_client, create_user, test_password):
    ...
    user = create_user()
    response = api_client.post(
        url, data={"email": user.email, "password": test_password}, format="json"
    )
