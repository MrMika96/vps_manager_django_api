...
class ApplicationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ["deployed_at", "updated_at"]
    ...
    def validate_deployed_to(self, deployed_to):
        ...
        if deployed_to:
            for vps in deployed_to:
                applications_size = (
                    sum(list(vps.deployed_applications.values_list("size", flat=True)))
        ...
        if vps_with_filled_hdd:
            raise ValidationError(
                detail=f"This servers don`t have enough "
    ...
    def update(self, instance, validated_data):
        validated_data.pop("vps_set")
        app = super(ApplicationSerializer, self).update(
...
class ApplicationShortSerializer(serializers.ModelSerializer):
    class Meta:
        model = Application
        fields = ["id", "title", "deployed_at", "updated_at"]
        read_only_fields = ["id", "title", "deployed_at", "updated_at"]
