...
class VpsSerializer(serializers.ModelSerializer):
    cpu = serializers.IntegerField(
        min_value=4, max_value=256, help_text="Number of CPU cores"
    )
    ram = serializers.IntegerField(
        min_value=4, max_value=6000, help_text="RAM value in gigabytes"
    )
    hdd = serializers.IntegerField(
        min_value=16, max_value=16000, help_text="HDD value in gigabytes"
    )
    ...
    free_space = serializers.FloatField(
        read_only=True, help_text="Free space of servers hdd (in gigabytes)"
    )
    free_space_percentage = serializers.FloatField(
        read_only=True, help_text="Free space of server's hdd in percentages"
    )
    ...
    def update(self, instance, validated_data):
        vps = super(VpsSerializer, self).update(
...
class VpsSingleSerializer(serializers.ModelSerializer):
    maintainers = MaintainerSerializer(
        source="maintained_by", many=True, read_only=True, default=[]
    )
    free_space = serializers.DecimalField(
        max_digits=5, decimal_places=2, read_only=True
    )
    free_space_percentage = serializers.DecimalField(
        max_digits=5, decimal_places=2, read_only=True
    )
    applications_size = serializers.DecimalField(
        max_digits=5, decimal_places=2, read_only=True
    )
    deployed_applications = ApplicationShortSerializer(
    ...
    class Meta:
        ...
        read_only_fields = ["id", "ram", "cpu", "hdd", "status"]
