...
class VpsFilter(django_filters.FilterSet):
    cpu = django_filters.NumberFilter(
        help_text="For finding vps with specific cpu value"
    )
    ...
    ram = django_filters.NumberFilter(
        help_text="For finding vps with specific ram value"
    )
    ...
    hdd = django_filters.NumberFilter(
        help_text="For finding vps with specific hdd value"
    )
    ...
    status = django_filters.ChoiceFilter(
        choices=Vps.STATUSES, help_text="Display all vps with selected status"
    )
