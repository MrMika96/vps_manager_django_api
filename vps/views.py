...
@extend_schema_view(
    list=extend_schema(
        description="View all of the existing vps in our system", summary="View vps"
    ),
    create=extend_schema(
        description="Add new vps to our system", summary="Create new vps"
    ),
    retrieve=extend_schema(
        description="View specific existing vps in our system",
        summary="View specific vps",
    ),
    update=extend_schema(
        description="Updates data about existing vps in our system",
        summary="Update existing vps",
    ),
    destroy=extend_schema(
        description="Deletion of existing vps from our system",
...
class VpsViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return qs.prefetch_related(
            ...
            free_space=Case(
                When(
                    applications_size__gt=0,
                    then=ExpressionWrapper(
                        F("hdd")
                        - F("applications_size")
                        / Value(1000, output_field=DecimalField()),
